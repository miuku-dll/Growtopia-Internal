...
std::vector<std::string> utils::ParseString(const std::string& str, std::string sep) {
...
std::string utils::RandomStringA(int len, std::string charset) {
    std::string buffer = "";
    for (int i = 0; i < len; i++)
...
std::wstring utils::RandomStringW(int len, std::wstring charset) {
    std::wstring buffer = L"";
    for (int i = 0; i < len; i++)
        buffer += charset[rand() % (charset.size() - 1)];
    return buffer;
}
std::string utils::Join(const std::vector<std::string>& strVector, std::string sep) {
    std::string buffer = "";
